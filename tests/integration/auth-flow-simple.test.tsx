...
describe('Integration Tests - Simple Authentication Flow', () => {
  beforeEach(() => {
    jest.clearAllMocks()
  ...
  describe('Complete Authentication Flow', () => {
    ...
    it('1.6.5 should maintain consistent auth state across multiple tabs', async () => {
      ...
      // Mock auth state change listener - shared across all tabs
      let authStateCallback: any = null
      mockOnAuthStateChange.mockImplementation((callback) => {
        authStateCallback = callback
        return {
      ...
      // Mock localStorage for cross-tab communication
      ...
      const TestComponent = ({ tabId }: { tabId: string }) => {
        ...
        return (
          <div>
            ...
            <div data-testid={`storage-value-${tabId}`}>
      ...
      // Render two tabs with shared context
      render(
        <AuthProvider>
      ...
      // Simulate auth state change (this affects the shared context)
      act(() => {
        authStateCallback('SIGNED_IN', mockSession)
      })

      // Both tabs should update to authenticated state (shared context)
      await waitFor(() => {
    ...
    it('1.6.6 should propagate auth state updates across tabs', async () => {
      ...
      // Simulate logout via auth state change (affects shared context)
      act(() => {
        authStateCallback('SIGNED_OUT', null)
      })

      // Both tabs should update to unauthenticated state (shared context)
      await waitFor(() => {
        expect(screen.getByTestId('user-status-tab1')).toHaveTextContent('not authenticated')
        expect(screen.getByTestId('user-status-tab2')).toHaveTextContent('not authenticated')
